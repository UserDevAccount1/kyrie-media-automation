    ...
    questions: [
      {
        ...
        answer: [
          ...
          'Worked example — "Florida": the brief\'s area is parsed to a geo entity (Florida, + city like Miami / Tampa / Orlando if given). The agent expands it to visual queries (Miami skyline aerial, palm-tree street, Florida waterfront home, suburb drone). Tier-1 stock (Pexels / Pixabay / Storyblocks) returns license-safe clips; opt-in Tier-2 scrape pulls geotagged #floridahomes / #miamirealestate social clips for true-local feel. A vision check confirms each clip actually looks like Florida (palms, FL architecture / signage) and drops wrong-location footage; remaining gaps are AI-generated ("Florida <scene>"). Results cache by region+topic so the next Florida video is near-instant.',
    ...
    questions: [
      ...
      },
      {
        ...
        answer: [
          ...
          'Equally valid option — US VPS / VPN (when set up properly): a US-hosted virtual server (or per-client US VPN) is a legitimate parallel path, NOT just a fallback. Set up correctly it works well: one isolated VPS/profile per client, a residential or clean US IP bound to that client (not one shared datacenter IP across accounts), and a matched browser fingerprint. Configured that way it stands alongside MoreLogin. The only thing to avoid is the lazy setup — a single shared datacenter IP for many clients — which is what triggers cross-account linking and bans. We also recommend a US VPS regardless to host the automation runtime (n8n, Hermes, render/orchestration) for US-local timing and clean official-API posts.',
        ],
